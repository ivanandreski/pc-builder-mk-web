...
export class RemotePcBuildRepository implements IPcBuildRepository {
  async fetchPcBuild(token?: string | null): Promise<PcBuild> {
...
// TODO: add bearer token and test if it works
  async addProduct(
    ...
    const { data } = await axios.post("customPcBuild/addProduct", {productSlug: product.slug});
  ...
  async removeProduct(
    ...
    const { data } = await axios.post("customPcBuild/removeProduct", {productSlug: product.slug});
